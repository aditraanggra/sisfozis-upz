...
class RebuildRekapPendis extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'pendis:rebuild 
     ...
     *
     * @param RekapPendisService $rekapitulasiService
     * @return int
     */
    public function handle(RekapPendisService $rekapitulasiPendis)
    {
        ...
        foreach ($units as $unit) {
            ...
            while ($currentDate <= $endDate) {
                ...
                if ($periode === 'all' || $periode === 'harian') {
                    $rekapitulasiPendis->updateDailyRekapPendis($currentDate, $unit->id);
                }
                ...
                if (($periode === 'all' || $periode === 'bulanan') && $currentDate->day === $currentDate->daysInMonth) {
                    $rekapitulasiPendis->updateMonthlyRekapPendis($currentDate->month, $currentDate->year, $unit->id);
                }
                ...
                if (($periode === 'all' || $periode === 'tahunan') && $currentDate->month === 12 && $currentDate->day === 31) {
                    $rekapitulasiPendis->updateYearlyRekapPendis($currentDate->year, $unit->id);
                }
